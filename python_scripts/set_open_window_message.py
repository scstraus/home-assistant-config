...
def make_open_windows_message(num_open_windows)
  ...
  window_counter = 0
  message=""
  if (hass.states.get("binary_sensor.back_door") == "on":
    message="The back door"
    counter = counter - 1
  if num_open_windows > counter:
    message = message + ", and windows in"
  elif num_open_windows > 1: 
    message = message + "Windows in"
  if (hass.states.get("binary_sensor.sebastians_room_left_window") == "on":
    message = message + " Sebastian's Room"
    counter = counter - 1
    window_counter = window_counter + 1
  if (hass.states.get("binary_sensor.sophies_room_ceiling_window") == "on":
    message = message + " Sophie's Room"
    counter = counter - 1  

"binary_sensor.ecolink_closet_left_window", "binary_sensor.laundry_room_window_zone_13", "binary_sensor.ecolink_garage_door"]
